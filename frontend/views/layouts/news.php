...
$this->registerMetaTag(['name' => 'copyright', 'content' => 'dentrangtri-saigon.com']);
$this->registerMetaTag(['name' => 'author', 'content' => 'dentrangtri-saigon.com']);
...
<body>
<?php $this->beginBody() ?><header><?= topHeader::widget() ?><?= mainMenu::widget() ?></header><section class="main"><div class="container-fluid"><div class="row">
    <?php if (Yii::$app->params['layout'] != '_mobile'): ?>
    <div class="col-lg-2 col-xl-2"><?php if (isset(Yii::$app->params['settingModules']['content']['content_left'])): ?><div class="left"><?php foreach (Yii::$app->params['settingModules']['content']['content_left']as $value): ?><?php switch ($value['type_module']) {
            ...
            default:echo verticalProductMenus::widget(['data' => $value]);
                break;
        } ?><?php endforeach ?></div><?php endif ?></div><?php endif ?><div class="col-lg-8 col-xl-8"><div class="center inside"id="content"><?= $content ?><?php $content_center = isset(Yii::$app->params['settingModules']['content']['content_center']) ? Yii::$app->params['settingModules']['content']['content_center'] : null;
if ($content_center) {
    foreach ($content_center as $value) {
        if ($value['type_module'] == 'custom') {
            ...
            default:echo verticalProductMenus::widget(['data' => $value]);
                break;
        } ?><?php endforeach ?></div></div><?php endif ?></div></div></section><?= footerWidget::widget() ?><?= \frontend\widgets\modalCartWidget::widget() ?><?php $this->endBody();
if (Yii::$app->session->hasFlash('messeage')) {
    $messeage = Yii::$app->session->getFlash('messeage');
}if (isset($messeage)): ?><script type="text/javascript">$.notify({icon: "pe-7s-gift", message: "<?= $messeage ?>"}, {type: "info", timer: 600})</script><?php endif; ?></body></html><?php $this->endPage() ?>
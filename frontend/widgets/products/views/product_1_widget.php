...
<?php if ($listProduct): ?>
<?php //dbg($data) ?>
<section class="module-navicat auto">
	<div class="header">
		<h2><?= $data['name'] ?></h2>
		<div class="viewallcat d-none d-sm-block">
			<?php foreach ($data['childrents'] as $value): //dbg($value) ?>
				<?php if ($value['type_module']=='product'||$value['type_module']=='news'): ?>
					<a href="<?= isset($value['cate_slug'])? Url::to(['product/index', 'slug' => $value['cate_slug']]) :''?>"><?= $value['name'] ?></a>
				<?php else: ?>
					<a href="a" title=""><?= $value['name'] ?></a>
				<?php endif ?>
			<?php endforeach ?>
	...
	<div class="owl-carousel owl-theme homepromo auto">
		<?php foreach ($listProduct as $product): $discount=false; //dbg($product) ?>
			<div class="item">
